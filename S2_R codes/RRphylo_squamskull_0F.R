...
setwd("D:/Programas/Rcodes/RRphylo/Squam_skull/PCs_95var/Squam_0F")

tree<- read.nexus("pruned_tree0.nex")
PCs_data <- read.csv("PCs-95var_0F.csv", header = TRUE)
...
tip_names<-tips(tree, node=93,labels=TRUE)
tip_values<-tips(tree, node=93,labels=FALSE)
...
write.csv(gmm_rates, "./All_rates_PCs5_0F.csv", row.names = FALSE)
...
write.csv(gmm_rates, file = "./All_rates_PCs_0F_Final.csv", row.names = FALSE)
...
#gmm_rates<-read.csv("All_rates_phyloPCs_0F_Final.csv")
#tree<- read.nexus("pruned_tree0.nex")
...
#Write post summary stats
write.csv(gmm_rates_clades, file = "All_rates_Squam0F_stats_RR_Final_Clades.csv", row.names = FALSE)
...
#gmm_rates_clades<-read.csv("./All_rates_Squam0F_stats_RR_Final_Clades.csv")
...
pl_rate
ggplot2::ggsave	("rates_0F_RelRate_dist.pdf", width=10, height=5)
...
pl_rate_norm<-  ggplot(gmm_rates_clades, aes(x=AllPCs_RelRateNorm)) +
  ...
  theme(plot.title = element_text(size = 2, face = "bold", hjust = 0.5))
pl_rate_norm
ggplot2::ggsave	("rates_0F_AllPCs_RelRateNorm_dist.pdf", width=10, height=5)
...
RR_92t_Rel<- clockrate_dens_plot(RateTable_Means_Clades, stack = TRUE, nrow = 2, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  ggplot2::scale_x_log10()+
  labs(x = "Log10 (relative rate mean)")
RR_92t_Rel
#ggplot2::ggsave	("RR_92t_RatesByClade_SkullSurface.pdf", width=8, height=6)


#Norm rates
## Stacked plots with viridis color scale
ggplot2::scale_colour_viridis_d(option = "turbo")+
  gRR_92t_N<- clockrate_dens_plot(RateTable_MeansNorm_Clades, stack = TRUE, nrow = 4, scales = "fixed")+
  gplot2::scale_fill_viridis_d(option = "turbo")+
  labs(x = " Relative rate mean (normalized)")
RR_92t_N
ggplot2::ggsave	("RR_92t_Rates_MeanNorm_ByClade_SkullSurface.pdf", width=8, height=6)


### Merge all Plots for BT 122-106-92t

plot_grid(RR_122t_N, RR_106t_N, RR_92t_N,
          labels = c('A', 'B', 'C'),
          label_size = 8, label_fontface = "bold",
          align = "hv", ncol = 1)

ggplot2::ggsave	("D:/Programas/Rcodes/PhyloParameters/Squamata_2024/Comparisons/RR_PC_All_Rates_MeanNorm_ByClade_SkullSurface.pdf", width=4, height=12)
...
p1
#ggsave("Tree92t_GMM_BL_Tips_RelRate(Circ).pdf", width=16, height=16)
...
p2 <- ggtree(tree_gmm_data, aes(color=AllPCs_RelRateNorm), layout = 'circular',
  ...
  theme(legend.position = "inside",
        ...
        legend.key.width=unit(1, "cm"))
p2
ggsave("Tree92t_GMM_BL_Tips_RelRatelLn(Circ).pdf", width=16, height=16)
...
p3 <- ggtree(tree_gmm_data, aes(color=AllPCs_RelRateNorm), layout = 'circular',
  ...
  theme(legend.position = "inside",
        ...
        legend.key.width=unit(1, "cm"))
p3
ggsave("Tree92t_GMM_BL_Tips_RelRateNorm(Circ).pdf", width=16, height=16)
...
Pl1a <- revts(Pl1a)
Pl1a
ggsave("Tree92t_GMM_BL_RelRateNorm_Tip+Clade(Rect).pdf", width=25, height=25)
...
ggsave("Tree92t_GMM_RelRateNorm_Clade(Circ_BL).pdf", width=14, height=14)
