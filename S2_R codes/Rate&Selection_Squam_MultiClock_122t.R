...
setwd("D:/Programas/Rcodes/PhyloParameters/Squamata_2024/4Mol4Mor_122t_100F")
...
################# TREE Prunning to match GMM sample #####################
#Get original tree with a full set of taxa
#@S4 class tree
tree_full <- read.beast("Comb_164t_SFBD_noSA_ILN_4Mol4Mor_FixTree_MCT.t.con.tre")
tree_subset <- read.nexus("pruned_tree100.nex")
keep_taxa <- tree_subset$tip.label


#Drop the tips not included in the reduced tree or keep taka list
#@S4 class tree
pruned_tree <- treeio::drop.tip(tree_full, setdiff(tree_full@phylo$tip.label, keep_taxa))

#Write out pruned annotated tree
#@S4 class tree
write.beast(pruned_tree, "MCT_122t.tre")
write.nexus(pruned_tree, "MCT_122t.nex")
...
tree<-treeio::read.mrbayes("MCT_122t.tre")
...
#write.csv(RateTable_Medians_no_clades, file="RateTable_Medians.csv", row.names = FALSE)
...
tree_norm<-tree
tree_norm@data <- tree@data %>% dplyr::mutate(across(where(is.numeric) & contains("rate"), normalize_ln_minmax))

RateTable_MeansNorm <- get_clockrate_table_MrBayes(tree_norm, summary = "mean")

#add age and other variables back to rate table
#RateTable_MeansNorm <- RateTable_MeansNorm[-c(1),] #remove root node (not comparable to other methods)
#age_median <- round(tree_norm@data$age_median[2:length(tree_norm@data$age_median)], digits=2) #get age medians rounded up

age_median <- round(tree_norm@data$age_median, digits=2)
age_median
RateTable_MeansNorm<- cbind(RateTable_MeansNorm,
...
ggplot2::ggsave	("MCT_122t_nodes.pdf", width=14, height=20)


#Specify the ancestral nodes and their string names
ancestral_nodes <- list(
  Other_Lepidosauria = c(128,4),
  Gekkota = 135,
  Scincoidea = 152,
  Lacertoidea = "164 - 179",
  Amphisbaenia = 179,
...
Clock4P_122t<-clockrate_dens_plot(RateTable_Means_Clades, clock = 5:8, stack = TRUE, nrow = 4, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  ggplot2::scale_x_log10()+
  labs(x = " Relative rate mean")
Clock4P_122t
ggplot2::ggsave	("Clock4P_122t_RatesByClade_Mean_P5-P8.pdf", width=6, height=12)


#Norm rates
## Stacked plots with viridis color scale
## Stacked plots with viridis color scale
Clock4P_122t_N<- clockrate_dens_plot(RateTable_MeansNorm_Clades, clock = 5:8, stack = TRUE, nrow = 4, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  labs(x = " Relative rate mean (normalized)")
Clock4P_122t_N
ggplot2::ggsave	("Clock4P_122t_RatesbyClade_MeanNORM_P5-P8.pdf", width=4, height=10)
...
Pl1a <- ggtree(tree, layout = "rectangular", ladderize=TRUE, right=TRUE, size=0.1) +
  ...
  theme_tree() +
  geom_cladelabel(node=124, label="Early Archosauriformes", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 0,
                  geom = "text", color = "black", fill = NA, offset=-200, offset.text=2) +
  geom_cladelabel(node=128, label="Sphenodontia", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=135, label="Gekkota", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=152, label="Scincoidea", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=165, label="Teiioidea", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=178, label="Lacert.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=179, label="Amph.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=203, label="Anguiformes", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=186, label="Acrodonta", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=191, label="Pleurodonta", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=220, label="Scolec.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=227, label="'Haenophidia'", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=233, label="Caenophidia", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4)
...
ggsave("Tree_4Mol4Mor_122t_Age&Probs_Clades.pdf", width=20, height=25)


#Ladder Tree - Rate log
tree_ln<-tree
tree_ln@data <- tree@data %>% dplyr::mutate(across(where(is.numeric) & contains("rate"), log))


pl_r1 <- ggtree(tree_ln, layout = "rectangular", ladderize=TRUE, right=TRUE,
                 aes(color=rateIlnBrlens8_mean), size=2)+
    geom_tiplab(size=4, linesize = 0.01, color="black", fontface = "italic", offset=1)+
    geom_tiplab(aes(x=branch, label=round(rateIlnBrlens8_mean, 0.5)), vjust=-.5, size=3, color="black")+
...
ggsave("4Mol4Mor_122t_RateMean_Ln_(Rect)_p8.pdf", width=20, height=25)
...
plot_clockrate_by_part_circ <- function(partition_number) {
  ...
  plot <- ggtree(tree_norm, layout = "circular", ladderize=TRUE, right=TRUE,
    ...
    geom_cladelabel(node=124, label="Archos.", align=TRUE,
    ...
    geom_cladelabel(node=178, label="Lacert.", align=TRUE,
                    barsize = 1.5, fontsize = 5.5, angle = "auto",horizontal = FALSE, hjust='center', offset.text = 6,
                    geom = "text", color = "black", fill = NA) +
    geom_cladelabel(node=179, label="Amphisb.", align=TRUE,
                    barsize = 1.5, fontsize = 5.5, angle = "auto",horizontal = FALSE, hjust='center', offset.text = 6,
                    geom = "text", color = "black", fill = NA) +
    geom_cladelabel(node=203, label="Anguiformes", align=TRUE,
                    barsize = 1.5, fontsize = 6, angle = "auto",horizontal = FALSE, hjust='center', offset.text = 6,
    ...
    geom_cladelabel(node=233, label="Caenophidia", align=TRUE,
                    barsize = 1.5, fontsize = 6, angle = "auto",horizontal = FALSE, hjust='center', offset.text = 6,
                    geom = "text", color = "black", fill = NA) +
    geom_strip(taxa1= 100, taxa2= 107, label="'Haenophidia'", align=TRUE,
               barsize = 1.5, fontsize = 6, angle = 60, horizontal = FALSE, hjust='center', offset.text = 10,
               geom = "text", color = "black", fill = NA)+
...
S1<- plot_treerates_sgn2(type = "MrBayes",
                         ...
                         layout = "circular",
                         clock = 5,               #Show rates for clock partition 1
...
## Save your plot to your working directory as a PDF
ggsave("4Mol4Mor_RateSign_p5_TEST.pdf", width=14, height=14)
...
combine_plots_ratesgn_spp <- function(partition_numbers) {
  ...
  master_plot <- cowplot::plot_grid(plotlist = plots, ncol = 2)
  ggsave("Morpho_AllParts_RateSign_Spp.pdf", master_plot, width=40, height=19 * length(partition_numbers)/2)
}
