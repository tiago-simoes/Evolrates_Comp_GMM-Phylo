...
setwd("D:/Programas/Rcodes/PhyloParameters/Squamata_2024/4Mol4Mor_92t_0F")
...
tree_subset <- read.nexus("pruned_tree0.nex")
...
#@S4 class tree
write.beast(pruned_tree, "MCT_92t.tre")
...
tree<-treeio::read.mrbayes("MCT_92t.tre") 
...
ggplot2::ggsave	("MCT_92t_nodes.pdf", width=14, height=20)


#Specify the ancestral nodes and their string names
ancestral_nodes <- list(
  Other_Lepidosauria = 1,
  Gekkota = 95,
  Scincoidea = 110,
  Lacertoidea = "122 - 130",
  Amphisbaenia = 130,
...
Clock4P_92t<-clockrate_dens_plot(RateTable_Means_Clades, clock = 5:8, stack = TRUE, nrow = 4, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  ggplot2::scale_x_log10()+
  labs(x = " Relative rate mean")
Clock4P_92t
ggplot2::ggsave	("Clock4P_92t_RatesByClade_Mean_P5-P8.pdf", width=6, height=12)


#Norm rates
## Stacked plots with viridis color scale
## Stacked plots with viridis color scale
Clock4P_92t_N<- clockrate_dens_plot(RateTable_MeansNorm_Clades, clock = 5:8, stack = TRUE, nrow = 4, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  labs(x = " Relative rate mean (normalized)")
Clock4P_92t_N
ggplot2::ggsave	("Clock4P_92t_RatesbyClade_MeanNORM_P5-P8.pdf", width=4, height=10)
...
plot_grid(Clock4P_164t_N, Clock4P_122t_N, Clock4P_106t_N, Clock4P_92t_N,
          labels = c('A', 'B', 'C', 'D'), 
          label_size = 8, label_fontface = "bold",
          align = "hv", ncol = 4)

ggplot2::ggsave	("Clock4P_All_Rates_MeanNorm_ByClade_SkullSurface.pdf", width=16, height=12)
...
Pl1a <- ggtree(tree, layout = "rectangular", ladderize=TRUE, right=TRUE, size=0.1) +
  ...
  theme_tree() +
  geom_cladelabel(node=95, label="Gekkota", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=110, label="Scincoidea", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=123, label="Teiioidea", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=130, label="Amph.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=150, label="Anguiformes", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=137, label="Acrodonta", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=139, label="Pleurodonta", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=160, label="Scolec.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=167, label="'Haenophidia'", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=173, label="Caenophidia", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4)
...
ggsave("Tree_4Mol4Mor_92t_Age&Probs_Clades.pdf", width=20, height=25)
...
ggsave("4Mol4Mor_92t_RateMean_Ln_(Rect)_p8.pdf", width=20, height=25) 
...
plot_clockrate_by_part_circ <- function(partition_number) {
  ...
  plot <- ggtree(tree_norm, layout = "circular", ladderize=TRUE, right=TRUE,
    ...
    geom_cladelabel(node=123, label="Teiioidea", align=TRUE,
                    barsize = 1.5, fontsize = 6, angle = "auto",horizontal = FALSE, hjust='center', offset.text = 6,
                    geom = "text", color = "black", fill = NA) +
    geom_cladelabel(node=130, label="Amphisb.", align=TRUE,
    ...
    geom_cladelabel(node=173, label="Caenophidia", align=TRUE,
                    barsize = 1.5, fontsize = 6, angle = "auto",horizontal = FALSE, hjust='center', offset.text = 6,
                    geom = "text", color = "black", fill = NA) +
    geom_strip(taxa1= 70, taxa2= 77, label="'Haenophidia'", align=TRUE,
               barsize = 1.5, fontsize = 6, angle = -17, horizontal = FALSE, hjust='center', offset.text = 10,
               geom = "text", color = "black", fill = NA)
...
#Comb_posterior <- combine_log("./P_files", burnin = 0.25, downsample = 2500)
