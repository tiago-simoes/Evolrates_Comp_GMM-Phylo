...
########################## Calculate evol rates with RRphylo #######################

setwd("D:/Programas/Rcodes/RRphylo/Squam_skull/PCs_95var/Squam_100F")

tree<- read.nexus("pruned_tree100.nex")
PCs_data <- read.csv("PCs-95var_100F.csv", header = TRUE)
...
tip_names<-tips(tree, node=123,labels=TRUE)
tip_values<-tips(tree, node=123,labels=FALSE)
...
write.csv(gmm_rates, "./All_rates_PCs_100F.csv", row.names = FALSE)
...
write.csv(gmm_rates, file = "./All_rates_phyloPCs_100F_Final", row.names = FALSE)
...
gmm_rates<-read.csv("All_rates_phyloPCs_100F_Final.csv")
tree<- read.nexus("pruned_tree100.nex")

######### Get clade Membership for each node in the phylogenetic tree (Part2)
...
ancestral_nodes <- list(
  ...
  Iguania = 132,
  Caenophidia = 170,
  Early_Serpentes = "164 - 170"
)


# Generate the data frame
Nodes_Clade_Table <- clade_membership(
  tree = "./pruned_tree100.nex",
...
#Write post summary stats
write.csv(gmm_rates_clades, file = "All_rates_Squam100F_stats_RR_Final_Clades.csv", row.names = FALSE)



############## Evolutionary Rates Statistics

#gmm_rates_clades<-read.csv("./All_rates_Squam100F_stats_RR_Final_Clades.csv")
...
pl_rate<-  ggplot(gmm_rates_clades, aes(x=AllPCs_RelRate)) +
  ...
  #xlim(c(0,quantile(gmm_rates$AllPCs_AllPCs_RelRate, 0.95))) +
  theme_bw() +
  theme(plot.title = element_text(size = 2, face = "bold", hjust = 0.5))
pl_rate

ggplot2::ggsave	("rates_100F_RelRate_dist.pdf", width=10, height=5)
...
pl_rate_norm<-  ggplot(gmm_rates_clades, aes(x=AllPCs_RelRateNorm)) +
  ...
  labs(x = "Rates", y = "Density") +
  #xlim(c(-quantile(gmm_rates_norm$AllPCs_RelRateNorm, 0.95),quantile(RateTable_Means_noCl_norm$AllPCs_RelRateNorm, 0.95))) +
  theme_bw() +
  theme(plot.title = element_text(size = 2, face = "bold", hjust = 0.5))
pl_rate_norm

ggplot2::ggsave	("rates_100F_AllPCs_RelRateNorm_dist.pdf", width=10, height=5)
...
RR_122t_Rel<- clockrate_dens_plot(RateTable_Means_Clades, stack = TRUE, nrow = 2, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  ggplot2::scale_x_log10()+
  labs(x = "Log10 (relative rate mean)")
RR_122t_Rel
#ggplot2::ggsave	("RR_122t_RatesByClade_SkullSurface.pdf", width=8, height=6)


#Norm rates
## Stacked plots with viridis color scale
RR_122t_N<- clockrate_dens_plot(RateTable_MeansNorm_Clades, stack = TRUE, nrow = 4, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  labs(x = " Relative rate mean (normalized)")
RR_122t_N
ggplot2::ggsave	("RR_122t_Rates_MeanNorm_ByClade_SkullSurface.pdf", width=8, height=6)
...
tree_gmm_rates <- full_join(tree, gmm_rates, by = 'node')


#rel rate
p1 <- ggtree(tree_gmm_rates, aes(color=AllPCs_RelRate), layout = 'circular',
...
p1
#ggsave("Tree122t_GMM_BL_Tips_RelRate(Circ).pdf", width=16, height=16)


#(circular) rel rate Ln  - BL & tip labels
p2 <- ggtree(tree_gmm_rates, aes(color=AllPCs_RelRateNorm), layout = 'circular',
  ...
  theme(legend.position = "inside",
        ...
        legend.key.width=unit(1, "cm"))
p2
ggsave("Tree122t_GMM_BL_Tips_RelRatelLn(Circ).pdf", width=16, height=16)

#(circular) rel rate normalized [-1,+1] - BL & tip labels
p3 <- ggtree(tree_gmm_rates, aes(color=AllPCs_RelRateNorm), layout = 'circular',
             ladderize = FALSE, size=3) +
  geom_tiplab(size=3.5, linesize = 0.01, color="black", fontface = "italic", hjust = -.1)+
  scale_color_gradientn(colours=c('blue','cyan3','orange',"red"),
                        na.value = "grey50",
  ...
  theme(legend.position = "inside",
        ...
        legend.key.width=unit(1, "cm"))
p3
ggsave("Tree122t_GMM_BL_Tips_RelRateNorm(Circ).pdf", width=16, height=16)



#(Ladder Tree) rel rate means normalized [-1,+1] - BL & tip labels
Pl1a<-ggtree(tree_gmm_rates, layout = "rectangular", ladderize=TRUE, right=TRUE,
  ...
  coord_geo(xlim=c(-330,60), ylim=c(0,Ntip(tree_gmm_rates)+1), expand=FALSE,
  ...
  geom_cladelabel(node=242, label="Archos.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=237, label="Sphenod.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=222, label="Gekkota", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=210, label="Scincoidea", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=192, label="Teiioidea", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=209, label="Lac.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=205, label="Amph.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=150, label="Anguiformes", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=145, label="Acrodonta", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=133, label="Pleurodonta", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=189, label="Scolec.", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=181, label="'Haenophidia'", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4) +
  geom_cladelabel(node=170, label="Caenophidia", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = 90, hjust='center',
                  geom = "text", color = "black", fill = NA, offset = 45, offset.text=4)
...
Pl1a <- revts(Pl1a)
Pl1a
ggsave("Tree122t_GMM_BL_RelRateNorm_Tip+Clade(Rect).pdf", width=25, height=25)



# Tree circular plot with mean rates - NO BL
pl1b <- ggtree(tree_gmm_rates, layout = "circular", ladderize=TRUE, right=TRUE,
               aes(color=AllPCs_RelRateNorm), branch.length="none", size=3) +
  scale_color_gradientn(colours=c('blue','cyan3','orange',"red"),
                        na.value = "grey50",
                        name = "Relative rates \n (normalized)")+
  theme_tree() +
  ...
  geom_cladelabel(node=209, label="Lac.", align=TRUE,
                  ...
                  geom = "text", color = "black", fill = NA) +
  geom_cladelabel(node=205, label="Amphisb.", align=TRUE,
                  barsize = 1.5, fontsize = 5.5, angle = "auto", horizontal = FALSE, hjust='center', offset.text = 7,
                  geom = "text", color = "black", fill = NA) +
  geom_cladelabel(node=150, label="Anguiformes", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = "auto", horizontal = FALSE, hjust='center', offset.text = 7,
  ...
  geom_cladelabel(node=170, label="Caenophidia", align=TRUE,
                  barsize = 1.5, fontsize = 6, angle = "auto", horizontal = FALSE, hjust='center', offset.text = 7,
                  geom = "text", color = "black", fill = NA) +
  geom_strip(taxa1= 115, taxa2= 21, label="'Haenophidia'", align=TRUE,
             barsize = 1.5, fontsize = 6, angle = 68, horizontal = FALSE, hjust='center', offset.text = 8,
             geom = "text", color = "black", fill = NA) +
  geom_strip(taxa1= 86, taxa2= 86, label="Mosasauria", align=TRUE,
             barsize = 1.5, fontsize = 6, angle = 0, horizontal = FALSE, hjust='center', offset.text = -50,
             geom = "text", color = "black", fill = NA)

pl1b

ggsave("Tree122t_GMM_RelRateNorm_Clade(Circ_noBL).pdf", width=14, height=14)


# Tree circular plot with mean rates - BL
pl1c <- ggtree(tree_gmm_rates, layout = "circular", ladderize=TRUE, right=TRUE,
  ...
  geom_cladelabel(node=170, label="Caenophidia", align=TRUE,
                  ...
                  geom = "text", color = "black", fill = NA) +
  geom_strip(taxa1= 115, taxa2= 21, label="'Haenophidia'", align=TRUE,
             barsize = 1.5, fontsize = 6, angle = 60, horizontal = FALSE, hjust='center', offset.text = 8,
             geom = "text", color = "black", fill = NA) +
  geom_strip(taxa1= 86, taxa2= 86, label="Mosasauria", align=TRUE,
             barsize = 1.5, fontsize = 6, angle = 0, horizontal = FALSE, hjust='center', offset.text = -50,
             geom = "text", color = "black", fill = NA)
...
ggsave("Tree122t_GMM_RelRateNorm_Clade(Circ_BL).pdf", width=14, height=14)
