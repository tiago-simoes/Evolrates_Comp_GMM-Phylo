...
##Post Process BayesTraits RJ-MCMC analysaes

setwd("D:/Programas/BayesTraits/Datasets/Squam(122t)/PCs_95var/delta/Squam_100F")

tree_gmm<- read.nexus("pruned_tree100.nex")

squam100F_rjpp <- rjpp(rjlog = "squam_100F_PCscores_run1.txt.VarRates.txt",
                     rjtrees =  "squam_100F_PCscores_run1.txt.Output.trees",
                     tree=tree_gmm,
                     burnin = 0, thinning = 10)


gmm_data<- squam100F_rjpp$data

write.csv(gmm_data, file = "./results_plots/Squam100F_BT_phyloPC_stats_raw.csv", row.names = FALSE)



############# calculate additional variables for data tables
setwd("D:/Programas/BayesTraits/Datasets/Squam(122t)/PCs_95var/delta/Squam_100F/results_plots")

gmm_data <- read.csv("./Squam100F_BT_phyloPC_stats.csv", header = TRUE)
...
write.csv(gmm_data, file = "Squam100F_stats_BT_phyloPC_Final.csv", row.names = FALSE)



######### Get clade Membership for each node in the phylogenetic tree
#(necessary for downstream analyses)
#gmm_data<-read.csv("Squam100F_stats_BT_phyloPC_Final.csv")
#tree_gmm<- read.nexus("pruned_tree100.nex")
...
ggsave("Tree_122t_NodeNo.pdf", width=20, height=22)
...
#Write post summary stats
write.csv(gmm_data_clades, file = "Squam100F_stats_BT_phyloPC_Final_Clades.csv", row.names = FALSE)
...
#gmm_data_clades<-read.csv("./Squam100F_stats_BT_phyloPC_Final_Clades.csv")
...
ggplot2::ggsave	("rates_100F_RelmeanRate_dist.pdf", width=10, height=5)
...
ggplot2::ggsave	("rates_100F_RelmeanRateNorm_dist.pdf", width=10, height=5)
...
BT_122t_Rel<- clockrate_dens_plot(RateTable_Means_Clades, stack = TRUE, nrow = 2, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  ggplot2::scale_x_log10()+
  labs(x = "Log10 (relative rate mean)")
BT_122t_Rel
#ggplot2::ggsave	("BT_phyloPC_122t_RatesByClade_SkullSurface.pdf", width=8, height=6)


#Norm rates
## Stacked plots with viridis color scale
BT_122t_N<- clockrate_dens_plot(RateTable_MeansNorm_Clades, stack = TRUE, nrow = 4, scales = "fixed")+
  ggplot2::scale_colour_viridis_d(option = "turbo")+
  ggplot2::scale_fill_viridis_d(option = "turbo")+
  labs(x = " Relative rate mean (normalized)")
BT_122t_N
ggplot2::ggsave	("BT_phyloPC_122t_Rates_MeanNorm_ByClade_SkullSurface.pdf", width=8, height=6)
...
############ Tree rate plots
#gmm_data<- read.csv(gmm_data, file = "./Squam100F_sum_stats_Final.csv", row.names = FALSE)
...
p1
#ggsave("BT_phyloPC_Tree122t_GMM_BL_Tips_RateMean_RelAbsolute(Circ).pdf", width=16, height=16)
...
p2 <- ggtree(tree_gmm_data, aes(color=RelmeanRateLn), layout = 'circular',
      ...
      theme(legend.position = "inside",
        legend.position.inside=c(.1, y=.1),
          ...
          legend.key.width=unit(1, "cm"))
p2
ggsave("BT_phyloPC_Tree122t_BL_Tips_RateMean_RelLn(Circ).pdf", width=16, height=16)
...
p3 <- ggtree(tree_gmm_data, aes(color=RelmeanRateNorm), layout = 'circular',
      ...
      theme(legend.position = "inside",
        legend.position.inside=c(.1, y=.1),
          ...
          legend.key.width=unit(1, "cm"))
p3
ggsave("BT_phyloPC_Tree122t_GMM_BL_Tips_RateMean_RelNorm(Circ).pdf", width=16, height=16)
...
Pl1a <- revts(Pl1a)
Pl1a
ggsave("BT_phyloPC_Tree122t_GMM_BL_Tips_RateMean_RelLn_Tip+Clade(Rect).pdf", width=25, height=25)



# Tree circular plot with mean rates - NO BL
pl1b <- ggtree(tree_gmm_data, layout = "circular", ladderize=TRUE, right=TRUE,
                 aes(color=RelmeanRateNorm), branch.length="none", size=3) +
    scale_color_gradientn(colours=c('blue','cyan3','orange',"red"),
                          na.value = "grey50",
                         name = "Relative rate mean \n (normalized)")+
    theme_tree() +
...
ggsave("BT_phyloPC_Tree122t_GMM_Tips_RateMean_RelLn_Clade(Circ_BL).pdf", width=14, height=14)
